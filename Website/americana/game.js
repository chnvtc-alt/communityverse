  ...
  function renderSetup() {
    ...
    const introCopy = replayCustomer
      ...
      : requestedReplayCustomerId
        ? "This is a replay for the character you selected. If you score higher, you can increase this character's reward. A lower score will not replace your best reward."
      : getOpenerCopy();
            ...
            requestedReplayCustomerId
              ? `
                <p class="helper opening-start-helper opening-title-helper">
                  You are playing again for <strong>${escapeHtml(replayCustomer?.name || "the selected character")}</strong>. Your best reward is protected.
                </p>
              `
              : ""
          }
      ...
      isFavoriteProgressReplay && collectionEntry.status !== "favorite"
        ? `
          <div class="favorite-progress-note">
            ...
            <p class="copy">Score <strong>${favoriteProgressMinScore}/10 or better</strong> with this character to build Favorite progress. After <strong>${favoriteGoal} successful visits</strong>, they become a Favorite Character and their score increases.</p>
  ...
  function renderResultPanel(session) {
              ...
              ? `
                <div class="hero-card result-followup-card result-followup-card-form" style="margin-top: 0; padding: 16px;">
                  ...
                  <form class="input-grid" id="profile-form" style="margin-top: 8px;">
                    ...
                    <p class="helper" style="margin: 0;">Other players only see your virtual restaurant name on leaderboards. We only use your email to send a secure save/sign-in link. Open your email and tap the link to finish saving with email. No password required.</p>
                    ...
                    <p class="helper ${state.emailInfoExpanded ? "" : "hidden"}" style="margin: 0;">${salesDemoMode ? "Email lets you recover your collection if you get a new phone, play on multiple devices, and avoid losing unlocked characters. We will not use your email for any other purpose." : "Email is the safest option. It lets you recover your scores and characters on another phone or browser. We will not use your email for any other purpose."}</p>
    ...
    if (isGuest && !state.showProfileForm) {
      ...
      form.addEventListener("submit", async (event) => {
        ...
        const playerName = String(activeProfile?.playerName || "Guest Player").trim();
